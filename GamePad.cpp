...
bool Gamepad::Refresh()
{
	...
	if (cId != -1)
	{
		...
		long normLX = max(-1, (float)state.Gamepad.sThumbLX / 32767);
		long normLY = max(-1, (float)state.Gamepad.sThumbLY / 32767);
		...
		long normRX = max(-1, (float)state.Gamepad.sThumbRX / 32767);
		long normRY = max(-1, (float)state.Gamepad.sThumbRY / 32767);
